...
def test_create_project_without_folder_returns_400():
    app = create_app()
    client = TestClient(app)
    res = client.post("/projects", json={"name": "NoFolder"})
...
def test_session_open_folder(tmp_path: Path):
    ...
    opened = client2.post("/session/open", json={"folder": str(tmp_path)}).json()
...
def test_get_project_reparses_stale_semantic_from_disk(tmp_path: Path):
    ...
    client = TestClient(app)
    project_id = client.post("/projects", json={"name": "Stale"}).json()["id"]
    add_content_file(
        client,
        project_id,
        tmp_path,
    ...
    assert any(
        v["name"] == "oldPort"
        for v in client.get(f"/projects/{project_id}").json()["semantic"].values()
    )
    ...
    refreshed = client.get(f"/projects/{project_id}").json()
...
def test_session_open_folder_missing_project_file(tmp_path: Path):
    app = create_app()
    client = TestClient(app)
    res = client.post("/session/open", json={"folder": str(tmp_path)})
