...
def _project(tmp_path: Path) -> tuple[TestClient, str]:
    ...
    project_id = client.post("/projects", json={"name": "Sheet"}).json()["id"]
...
def test_put_and_delete_title_block(tmp_path: Path):
    ...
    updated = client.put(
        f"/projects/{project_id}/sheet/title-block",
        json=body,
    ).json()
    assert updated["sheet"]["titleBlock"]["title"] == "Vehicle IBD"
    assert updated["sheet"]["titleBlock"]["position"] == "bottom-right"

    cleared = client.delete(f"/projects/{project_id}/sheet/title-block").json()
...
def test_put_and_delete_frame(tmp_path: Path):
    client, project_id = _project(tmp_path)
    updated = client.put(
        f"/projects/{project_id}/sheet/frame",
    ...
    cleared = client.delete(f"/projects/{project_id}/sheet/frame").json()
